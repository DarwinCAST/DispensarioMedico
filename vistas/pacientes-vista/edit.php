...
$sql = $conexion->query("select * from gestion_pacientes where id_paciente = $id");
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="content">
        <div class="form-container">
            ...
            <form method="POST" action="">
                ...
                <?php
                include("/laragon/www/ProyectoDispensarioMedico/controladores/Pacientes/EditPaciente.php");

                while ($datos = $sql->fetch_object()) { ?>
                    <div class="mb-3">

                        <label for="nombre" class="form-label">Nombre</label>
                        <input type="text" value="<?= $datos->nombre_paciente ?>" class="form-control" id="nombre" name="nombre" required>
                    </div>
                    <div class="mb-3">
                        <label for="cedula" class="form-label">Cedula</label>
                        <input type="text" value="<?= $datos->cedula_paciente ?>" class="form-control" id="cedula" name="cedula" required>
                    </div>
                    <div class="mb-3">
                        <label for="carnet" class="form-label">No-Carnet</label>
                        <input type="text" value="<?= $datos->no_carnet ?>" class="form-control" id="carnet" name="carnet" required>
                    </div>
                    <div class="mb-3">
                        <label for="tipo_paciente" class="form-label">Tipo de paciente</label>
                        <select class="form-control" id="tipo_paciente" name="tipo_paciente" required>
                            <option value="Indeterminado" <?= ($datos->tipo_paciente == 'Indeterminado') ? 'selected' : '' ?>>Indeterminado</option>
                            <option value="Bueno" <?= ($datos->tipo_paciente == 'Bueno') ? 'selected' : '' ?>>Bueno</option>
                            <option value="Indeterminado" <?= ($datos->tipo_paciente == 'Regular') ? 'selected' : '' ?>>Regular</option>
                            <option value="Indeterminado" <?= ($datos->tipo_paciente == 'Grave') ? 'selected' : '' ?>>Grave</option>
                            <option value="Indeterminado" <?= ($datos->tipo_paciente == 'Critico') ? 'selected' : '' ?>>Critico</option>
