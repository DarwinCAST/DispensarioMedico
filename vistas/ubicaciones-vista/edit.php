...
$sql = $conexion->query("select * from gestion_ubicaciones where id_ubicacion = $id");
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="content">
        <div class="form-container">
            ...
            <form method="POST" action="">
                ...
                <?php
                include("/laragon/www/ProyectoDispensarioMedico/controladores/UbicacionesFarmacos/EditUbicacion.php");

                while ($datos = $sql->fetch_object()) { ?>
                    <div class="mb-3">
                        <label for="ubicacion" class="form-label">Ubicacion del farmaco</label>
                        <input type="text" class="form-control" id="ubicacion" name="ubicacion" value="<?= $datos->ubicacion ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="estante" class="form-label">Estante del farmaco</label>
                        <input type="text" class="form-control" id="estante" name="estante" value="<?= $datos->estante ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="tramo" class="form-label">Tramo</label>
                        <input type="text" class="form-control" id="tramo" name="tramo" value="<?= $datos->tramo ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="casilla" class="form-label">Casilla</label>
                        <input type="text" class="form-control" id="casilla" name="casilla" value="<?= $datos->casilla ?>" required>
                    </div>
                <?php }
                ?>
